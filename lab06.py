...
# Game Flow
# Load the last game result from save.txt
try:
    with open("save.txt", "r") as file:
        lines = file.readlines()
        last_result = lines[-1].strip() if lines else None
        if last_result:
            print(f"Previous game result: {last_result}")
...
if not input_invalid:
    ...
    if last_result:
        if "Hero" in last_result:
            try:
                stars = int(last_result.split()[-2])  # Extract number of stars
                if stars > 3:
                    m_combat_strength += 1
            except ValueError:
                pass
        elif "Monster" in last_result:
            combat_strength += 1
...
# Display final score
    print(f"Game Over! Hero received {num_stars} stars.")
