...
m_combat_strength = 3  # Example initial combat strength
...
#4: Define loot options and the player's belt
loot_option = ["Sword", "Shield", "Potion", "Cursed Amulet", "Elixir"]
belt = []
...
def collect_loot():
    ...
    if loot_option:
        item = random.choice(loot_option)
        loot_options.pop(loot_option.index(item))
        belt.append(item)
        print(f"You obtained: {item}")
        print("Current belt contents:", belt)
    else:
...
print("Organizing belt...")
belt.sort()
print("Organized belt:", belt)

#8: Use the first loot item
if belt:
    print("You see a monster in the distance! You decide to use an item from your belt.")
    used_item = belt.pop(0)
    print(f"You used {used_item}!")

    # Health system
    health_points = 4  # Example initial health points
    good_loot = ["Sword", "Shield", "Potion", "Elixir"]
    bad_loot = ["Cursed Amulet"]

    if used_item in good_loot:
        health_points = min(health_points + 2, 6)
        print("The item was helpful! Your health increased.")
    elif used_item in bad_loot:
        health_points = max(health_points - 2, 0)
        print("The item was cursed! Your health decreased.")
    else:
        print("The item had no effect.")

    print(f"Updated health points: {health_points}")
else:
    print("Your belt is empty! You have nothing to use.")
